...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 15 13:44:40 2018
...
def url_assembler():
    prefix = "https://api.archives-ouvertes.fr/search/?q=*:*&wt=xml"
    ...
    numberOfResults="rows=18"
    return prefix+"&"+fields+"&"+numberOfResults

# renvoie une string à partir d'une string url.
request = URL.request

# crée un objet BeautifulSoup à partir d'une string en XML
def get_XML_SOUP(XML_string):
    XML_SOUP = BeautifulSoup(XML_string, "xml")
    return XML_SOUP
...
get_title = finder_generator('arr', 'name', "title_s")
...
get_structIsChildOf = finder_generator('arr', 'name', "structIsChildOf_fs")
...
def get_global_Structure_to_Parent_MAP(doc):
    Strings_iterator = get_structIsChildOf(doc)
...
def collate_doc_data(doc):
    collated_doc={}
    collated_doc["url"]=get_pdf_url(doc)
...
def collate_request_data(url):
    List_Docs=get_List_Docs(get_XML_SOUP(request(url)))
    List_collated_docs=[collate_doc_data(doc) for doc in List_Docs]
...
def request_to_file(url, file_name):
    json_to_file(collate_request_data(url), file_name)
