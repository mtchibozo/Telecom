...
# -*- coding: utf-8 -*-
"""
Utilise les méthode de DataCollectionFromHAL pour tracker les affiliations d'un auteur au cours des années.
"""
...
def assemble_author_search_url(author_name):
...
def update_selective_author_tracker(collated_doc, track_MAP, author_name):
    ...
    if author_name in affiliation_MAP:
        affiliation_SET=dc.tree_to_path_strings(dc.root_affiliations(affiliation_MAP[author_name], Affiliation_Parent_MAP), Affiliation_Parent_MAP)
...
#dc.request_to_file(url, file_name) # done once is enough
List_collated_docs=dc.lists_to_sets_copy(dc.json_to_python(file_name))
...
for year in range (1988, 2016):
    ...
    if year in MAP:
        print(year)
        print(MAP[year])
        print("\n=================================================\n")#dc.request_to_json(url, file_name)
